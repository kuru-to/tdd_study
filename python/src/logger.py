...
path_config = config_ini["DEFAULT"]["PATH_CONFIG"]
logging.config.fileConfig(f"{path_config}logging.conf")


def add_postfix_log(filename: str):
    """ファイル名に `.log` とついていなければつけて出力"""
    postfix = ".log"
...
def setup_logger(
    ...
    *,
    ...
    path_log: str = config_ini["DEFAULT"]["PATH_LOG"],
    ...
    logfile = add_postfix_log(f"{path_log}{str_batch_date}_{name}")
